...
export class FormatDataChartService {
  ...
  private getOrderedDates(
    ...
    switch (rangeType) {
      ...
      case 'monthly':
        ...
        first = `${startDate.getUTCFullYear()}/${
          startDate.getMonth() + 1
        }/${startDate.getDate()}`;
        last = `${endDate.getUTCFullYear()}/${
          endDate.getMonth() + 1
        }/${endDate.getDate()}`;
  ...
  private getDailyOrderedData(
    ...
    orderedDate.forEach((date, index, array) => {
      ...
      let valueForActiveDate = data.find(
        (el) => el.date === activeFormattedDate
      );
  ...
  private getMonthlyOrderedData(
    ...
    orderedDate.forEach((date, index, array) => {
      if (date === '') {
        const addMonth = new Date(
          activeMonth.date.setMonth(activeMonth.date.getMonth() + 1)
        );
        activeMonth = {
          date: addMonth,
      ...
      let valueForAcriveMonth = data.filter((el) => {
        ...
        if (
          ...
          activeMonth.year === date.getUTCFullYear()
        ) {
          return true;
        } else {
          return false;
  ...
  formatData(
    ...
    switch (rangeType) {
      case 'weekly':
        orderedData = this.getDailyOrderedData(
          dataByDay,
          orderedDate,
          startDate,
          endDate
      ...
      case 'monthly':
        ...
        break;
      case 'annual':
        orderedData = this.getMonthlyOrderedData(
          dataByDay,
          orderedDate,
          startDate,
          endDate
